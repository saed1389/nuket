    ...
    <section class="blog-detail-banner-section">
        ...
        <div class="auto-container">
            <ul class="page-breadcrumb">
                ...
                <li><a href="/workshop">{{ __('homePage.workshops') }}</a></li>
                <li>@if($lang == 'tr') {{ $workshop->title }} @else {{ $workshop->{ 'title_'.$lang } }} @endif</li>
            </ul>
            <div class="content-box">
                <h2>@if($lang == 'tr') {{ $workshop->title }} @else {{ $workshop->{ 'title_'.$lang } }} @endif <a href=""><img src="{{ asset('assets/images/log.png') }}" alt="" style="width: 50px;"></a></h2>
                <div class="clearfix">
                    <div class="pull-left">
                        ...
                        <div class="inner-box">
                            <div class="image-column col-lg-6 col-md-6 col-sm-12">
                                <div class="inner-column">
                                    <div class="image">
                                        <img class="rounded" src="{{ url('storage', $workshop->image) }}" alt="">
                                    </div>
                                </div>
                            </div>
                            <div class="inner-box mt-5">
                                <p>
                                    @if($lang == 'tr') {!! $workshop->description !!} @else {!! $workshop->{'description_'. $lang} !!} @endif
                                </p>
                            </div>
                        </div>
                    </div>
                                ...
                                <div class="widget-content">
                                    <article class="post">
                                        <figure class="post-thumb">
                                            ...
                                            <a href="@if($lang == 'tr') {{ '/workshop/'.$item->slug }} @else {{ '/workshop/'.$item->{'slug_' . $lang} }} @endif" class="overlay-box">
                                                <span class="icon fa fa-link"></span>
                                            </a>
                                        </figure>
                                        <div class="post-info">{{ $item->created_at->diffForHumans() }}</div>
                                        <div class="text">
                                            <a href="@if($lang == 'tr') {{ '/workshop/'.$item->slug }} @else {{ '/workshop/'.$item->{'slug_' . $lang} }} @endif">@if($lang == 'tr') {{ $item->title }} @else {{ $item->{ 'title_'.$lang } }} @endif </a>
                                        </div>
                                    </article>
                                </div>
                            @endforeach
